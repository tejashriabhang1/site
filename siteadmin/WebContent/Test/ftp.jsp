...
<form name="unitform" action="" method="get">
	...
	<table border="0" width="100%" align="center" class="sortable">
		<tr>
			<td colspan="10" align="center" class="sorttable_nosort"><b>Please
					select the search option to see the FTP data.</b></td>
		</tr>
		<tr>
			<td bgcolor="#F5F5F5"><label> <input type="radio"
					name="Search" onclick="SearchCrit(0);" />Unit id
			</label></td>
			<td bgcolor="#F5F5F5"><label> <input type="radio"
					name="Search" onclick="SearchCrit(1);" checked /> ALL
			</label> <input type="hidden" name="flag" id="flag" value="false"></td>

			<td bgcolor="#F5F5F5"><input type="text" name="unitid"
				id="unitid" class="stats" disabled="disabled"></input></td>
...
<tr>
	...
	<!-- <td colspan="13"> -->
		<%
			if (!(null == request.getQueryString())) {
					...
					if (flag.equalsIgnoreCase("true")) {
						...
						message = "The FTP Data of unit " + unitid;
						String rid = "";
						String ssql = "select rid,max(StoredDateTime) as rr from db_gps.t_ftplastdump where unitid in ("
								+ unitid + ") group by UnitID order by  StoredDateTime";
						...
						if (rid == "" || rid == null) {
							...
							sql = "select * from t_ftplastdump where StoredDateTime in (" + rid
									+ ") order by StoredDateTime desc ";
		...
		<table border="0" width="100%" align="center">
			<tr bgcolor="#2696B8">
				<th colspan="27" class="hed" bgcolor="#2696B8" align="center"><font
					color="white" size="1"><b><%=message%> </b></font>
					<div align="right">
						<a href="excelftpdumpdata.jsp?sql=<%=sql%>&message=<%=message%>"
