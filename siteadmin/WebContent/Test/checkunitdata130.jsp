	...
	//	String sqlUnitTyp = "select * from db_gps.t_unitmaster where unitid='"+unitid+"'";
		...
		if(rs.next()){
			...
			if("IP".equalsIgnoreCase(rs.getString("UnitType"))){
				sqlProcessed="SELECT StoredTime,StoredDate,VehRegNo,Vehid,Transporter,Body,UnProcessedStamps,MailFrom FROM t_ip"+thedate2+"  where Unitid='"+unitid+"'  UNION ALL select StoredTime,StoredDate,VehRegNo,Vehid,Transporter,Body,UnProcessedStamps,MailFrom from t_ip"+thedate2+"Processed where Unitid='"+unitid+"' order by StoredDate desc ,StoredTime  desc "+limit;	
	
				System.out.println("innnnnn"+sqlProcessed);

			}
		else
		{
			sqlProcessed="SELECT StoredTime,StoredDate,VehRegNo,Vehid,Transporter,Body,UnProcessedStamps,MailFrom FROM t_mails"+thedate2+"  where Unitid='"+unitid+"'  UNION ALL select StoredTime,StoredDate,VehRegNo,Vehid,Transporter,Body,UnProcessedStamps,MailFrom from t_mails"+thedate2+"Processed where Unitid='"+unitid+"' order by StoredDate desc ,StoredTime desc "+limit;
			System.out.println("elseeeeee"+sqlProcessed);
		...
		%>
			...
			<td  align="center"><%=rst.getDate("StoredDate") %></td>
