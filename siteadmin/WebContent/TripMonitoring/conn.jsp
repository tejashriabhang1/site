<%@ page language="java" import="java.sql.*" import="java.util.*" import=" java.text.*" import="javax.swing.*" import="moreservlets.listeners.*" errorPage="" %>
<%!
String MM_dbConn_DRIVER="org.gjt.mm.mysql.Driver";
/*
String MM_dbConn_USERNAME="java";
String MM_dbConn_PASSWORD="transjava!1";
*/
String MM_dbConn_USERNAME="site";
String MM_dbConn_PASSWORD="1@s2te";

String MM_dbConn_STRING="jdbc:mysql://10.0.10.62/db_gps";
String MM_dbConn_STRING3="jdbc:mysql://10.0.10.62/db_gpsmreports";
String MM_dbConn_STRING1="jdbc:mysql://10.0.10.62/db_CustomerComplaints";

//St ring MM_dbConn_STRING2="jdbc:mysql://10.0.10.62/airtel";
/* 
String MM_dbConn_STRING="jdbc:mysql://10.0.10.62/db_gps";
String MM_dbConn_STRING3="jdbc:mysql://10.0.10.62/db_gpsmreports";
String MM_dbConn_STRING1="jdbc:mysql://10.0.10.62/db_CustomerComplaints";

*/
//----------------------------------------------------------------------------------
//----------------------------------------------------------------------------------



%>

